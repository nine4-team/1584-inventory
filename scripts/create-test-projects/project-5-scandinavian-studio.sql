-- Create Test Project (Scandinavian Studio) with Transactions and Items
...
-- ============================================
  ...
  INSERT INTO projects (
    ...
    v_account_id,
    'Scandinavian Studio',
    'Light-filled studio apartment with Scandinavian design principles: light woods, simple forms, and functional beauty',
    'Emma Johansson',
    105000.00,
    26250.00,
    '{"designFee": 26250, "furnishings": 65000, "install": 5000, "kitchen": 0, "fuel": 2000, "storageReceiving": 3000, "propertyManagement": 3750}'::jsonb,
  ...
  v_raw_purchase := 800.00;
  v_raw_project := 900.00;
  v_raw_market := 1000.00;
  ...
  INSERT INTO items (
    ...
    v_account_id, v_project_id, v_tx_id, v_item_id, 'Sleek Sofa',
    'Light gray fabric sofa, minimalist design, 2-seater',
    'HG-SOFA-SCANDI-001', 'IKEA',
  ...
  INSERT INTO items (
    ...
    v_account_id, v_project_id, v_tx_id, v_item_id, 'Dining Table',
    'Light oak dining table, extends to seat 6',
    'HG-TABLE-OAK-001', 'IKEA',
    v_purchase::text, v_project_price::text, v_market::text,
    v_tax_rate_pct, v_tax_purchase::text, v_tax_project::text,
    'Client Card', 'purchased', v_qr_key, 'allocated', CURRENT_DATE,
    v_created_by, NOW(), NOW()
  ...
  INSERT INTO transactions (
    ...
    v_account_id, v_project_id, v_tx_id, CURRENT_DATE - INTERVAL '75 days', 'IKEA', 'Purchase',
    v_tx_total::text, 'Furnishings purchase from IKEA', v_cat_furnishings, 'completed', 'Client Card',
    v_tax_rate_pct, v_tx_subtotal::text, v_item_ids,
    v_created_by, NOW(), NOW()
  );
  
  RAISE NOTICE 'Project 5 created: %', v_project_id;
