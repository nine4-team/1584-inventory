...
- [ ] **Offline transaction create → sync drops `sum_item_purchase_prices`.** Repro: create a transaction offline (with or without child items). When connectivity returns, `executeCreateTransaction` loads the cached record and inserts `sum_item_purchase_prices = null`, which violates the DB NOT NULL constraint and the optimistic transaction disappears. Guard rails we still need:
  - **Persist a real sum before queueing.** In `offlineTransactionService.createTransaction`:
    - Initialize `let sum = 0` and, after each optimistic child item is created, add `parseFloat(item.purchasePrice || item.projectPrice || '0')` (treat NaN as 0). Keep the running total in cents or as a float but convert to a two-decimal string via `sum.toFixed(2)` before calling `offlineStore.saveTransactions`.
    - Even when there are zero child items, explicitly set `tempTransaction.sumItemPurchasePrices = '0.00'`. Never leave it `undefined`—that’s what causes the executor to send `null`.
